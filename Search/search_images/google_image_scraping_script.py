...
def fetch_image_urls(query: str, max_links_to_fetch: int, wd: webdriver, sleep_between_interactions: int = 1):
    ...
    # "https://www.google.com/search?q={q}&tbm=isch&hl=ar&tbs=il:ol&sa=X&ved=0CAAQ1vwEahcKEwiQ-5CG9aTzAhUAAAAAHQAAAAAQAg&biw=1903&bih=880#imgrc=xdmbjKc2IQrZaM"
...
if __name__ == '__main__':
    ...
    queries = pd.read_csv("../products.csv").iloc[:, 1]
    for query in queries:
        wd.get('https://google.com')
        search_box = wd.find_element_by_css_selector('input.gLFyf')
        search_box.send_keys(query)
        links = fetch_image_urls(query, 1, wd)  # 200 denotes no. of images you want to download
        images_path = 'dataset/'
        for i in links:
